...
class CityValidator:
    """
    Класс для валидации данных о городе.
    Получаем словарь на проверку и возвращает True, если данные валидны, иначе False
    """
    ...
    def __validate(self):
        ...
        Если проверка не проходит, возвращает False

        :return: True, если данные валидны, иначе False
        """
        validate(self.__city_data, self.json_schema)

        return True

    def __call__(self, city_data: dict) -> bool:
...
class CitySerializer:
    """
    Класс для десериализации данных о городн.
    Композиция: создается экземпляр класса валидации при инициализации класса сериализации
    Принимаем словарь, прогоняем через валидатор и возвращает экземпляр дата-класса City
    """

    def __init__(self):
        self.__validator = CityValidator()

    def __call__(self, city_data: dict) -> City:
        self.__validator(city_data)
